...
We enriched not just the URLs that each model cited but also the top-10 SERP results that were \emph{not} selected. This lets us compare what the model had available---the retrieval pool, or ``menu''---with what it actually cited---the ``order''---and ask whether models show systematic structural preferences when selecting sources.

\subsection{The Retrieval Pool}
\label{sec:menu}
\label{sec:order}

Tables~\ref{tab:menu-combined} and~\ref{tab:cited-combined} show the type distribution across the full retrieval pool and the cited-only subset, respectively.

\begin{table}[H]
\centering
\caption{Retrieval pool (``Menu'') type distribution}
...
\begin{table}[H]
\centering
\caption{Cited set (``Order'') type distribution}
...
Roughly 75\% of the retrieval pool consists of listicles and product pages, and the cited set mirrors this. Gemini leans heavier on listicles (56.8\% of citations vs.\ $\sim$37\% for GPT), consistent with its year-injected fan-out queries pulling in more ``Best X of 2026'' pages (Section~\ref{sec:freshness-subsection}) as opposed to product pages. Within each system, though, the menu-to-order shift in type proportions is modest---neither model radically favours one page type over another. Selection preferences instead show up in structural features \emph{within} those types, which we quantify next.

\subsection{Selection Drift}
\label{sec:selection-drift}
\label{sec:cited-vs-serp}

For each run, every SERP URL (filtered by content type) is classified as Cited or Not-Cited. Each run is an independent selection event: if a URL is cited in run~1 but not run~2, it appears in the Cited group for run~1 and the Not-Cited group for run~2, with zero overlap within any single run. We test eight binary Content DNA features using Welch's two-sample $t$-test (\texttt{freshness\_cue\_strength} binarized at $\geq$3). A per-unique-URL robustness check (deduplicating across runs) produces the same direction of drift for every significant feature.
...
\begin{tabular}{lrrrr}
\toprule
 & \makecell[r]{GPT Business\\Bing} & \makecell[r]{GPT Personal\\Bing} & \makecell[r]{GPT Personal\\Google} & \makecell[r]{Gemini\\Google} \\
\midrule
...
Across conditions, both models over-select listicles that contain tables, numbered lists, and vendor-owned content. GPT Personal vs Google shows the largest effects (Tables $+$19.7\,pp, $p<0.001$). ChatGPT also shows strong freshness drift (up to $+$16.6\,pp for Personal vs Bing), while Gemini shows none---because 96.7\% of Gemini's fan-out queries already inject year tokens (Section~\ref{sec:freshness-subsection}), pre-filtering the SERP for freshness before selection happens. Clear Authorship diverges directionally: ChatGPT under-selects pages with bylines across all conditions (not significant under the SERP constraint), while Gemini over-selects them ($+$5.9\,pp, $p<0.05$).
...
Product pages show flatter drift profiles. They are structurally homogeneous---high vendor-owned rates, low table and pros/cons rates---leaving less for either model to differentiate on. Numbered lists are the most consistent signal, and Gemini product pages show significant under-selection of pages with editorial sources/citations ($-$3.6\,pp, $p<0.001$).

\subsection{Cited vs Additional --- Structural Comparison}
\label{sec:cited-vs-additional}

The flip-flop analysis (Section~\ref{sec:flip-flop}) showed that one in five additional URLs switches to cited on the same prompt across runs. Are cited and additional links structurally different, or is the boundary arbitrary? Both groups were surfaced by ChatGPT: cited URLs appeared in the response text, additional URLs were shown as supplementary sources. Gemini lacks an ``additional'' category, so this comparison is GPT-only.
