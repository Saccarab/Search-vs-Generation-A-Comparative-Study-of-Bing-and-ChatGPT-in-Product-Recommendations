...
Flip-flop rate (cross-prompt) & 25.96\% & 25.91\% \\
\midrule
Same-prompt flip-flop URLs & 262 & 346 \\
Flip-flop rate (same prompt) & 20.2\% & 21.7\% \\
\bottomrule
\end{tabular}
\end{table}
...
(25.96\% vs.\ 25.91\%). On the same prompt across different runs,
one in five additional URLs switches to cited. The boundary between ``cite inline'' and ``show as supplementary''
is not stable---and this reshuffling concentrates among product
pages and listicles, predominantly the commercial content types that brands
compete over most directly.
