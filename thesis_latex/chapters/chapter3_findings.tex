...
Invisible links from different source categories (Section~2.3.2) play different roles in a response, however.
...
Rejected URLs can also be promoted, though at lower rates (8--17\%) and with a different profile: news articles and reference pages (primarily Wikipedia) dominate, and when promoted they skip the additional category almost entirely, going straight to cited.
...
Structured content features (tables, numbered lists, bullet points) are over-selected across almost all conditions. Gemini shows no significant freshness drift; this is consistent with its fan-out queries injecting year tokens that pre-filter the SERP for recency before selection occurs (Section~\ref{sec:freshness-subsection}). GPT Plus shows a robust freshness preference that is significant against both Bing and Google independently, while GPT Business shows none. Vendor-owned pages are also consistently over-selected, suggesting that brands produce more structurally thorough content that LLMs prefer, possibly because the models are cautious about surfacing commercial results that lack supporting detail.
