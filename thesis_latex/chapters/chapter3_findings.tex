...
The Page~2 dip on both Bing charts is consistent with the pagination instability documented in Section~\ref{sec:bing-pagination-instability}. For Business, the Page~1 concentration is striking: even though the median Page~1 contained only 3 organic results per query (Section~\ref{sec:bing-pagination-instability}), it still accounts for the largest share of citation matches by a wide margin. Top-ranked Bing results drive citations. Personal's flatter Bing profile is harder to interpret because our scrape used the consumer UI, not the Bing Search API that ChatGPT actually queries. With API access, Personal's Bing distribution would likely show more top-rank concentration as well---just at lower volume than Business, since Personal splits its retrieval across engines.
...
To quantify this parallel, we restrict to Page~1 results and compute each rank's share of all Page-1 matches. For GPT Personal, we take the best (highest) rank each citation achieves across both indices to avoid double-counting. Bing data excludes runs affected by the scraper artifact (Section~\ref{sec:bing-page-dist}); ranks 6--10 use only runs where Bing Page~1 had 8+ results.

Table~\ref{tab:cross-rank-ctr} places the cross-system rank concentration alongside Google organic click-through rates reported by First Page Sage \parencite{firstpagesage2026ctr}.
...
Gemini shows the steepest rank-1 concentration at 23.0\%, dropping to 13.4\% at rank~2. GPT Business spreads its weight across ranks 1--2 (17.4\% and 18.7\% respectively, 36.1\% combined), while GPT Personal is the flattest, ranging from 18.1\% at rank~1 to 9.4\% at rank~5---likely because drawing from two indices creates more rank diversity.

All three LLM profiles are flatter than human click-through behavior: the top-3 ranks account for 45--50\% of Page-1 matches for LLMs versus 68.7\% of clicks for humans. An important caveat: the denominators differ. Human CTR sums to $\sim$94\% across ranks 1--10 because humans rarely go past Page~1. The LLM columns sum to 100\% only because they are restricted to Page-1 matches by definition; as Table~\ref{tab:bing-page-dist} showed, many citations match beyond Page~1. The comparison therefore shows the shape of rank preference within Page~1, not overall reliance on it.
