...
\section{Related Work: SEO, GEO, and Generative Search}
\label{sec:related-work}

This section situates the thesis within the emerging literature on how generative systems mediate visibility on the web. We focus on work that is directly relevant to commercial discovery: (i) conceptual frameworks for \textit{Generative Engine Optimization} (GEO), (ii) empirical studies of which sources generative systems select and cite, and (iii) early evidence on how generative interfaces change the incentives and outcomes of traditional SEO.

\subsection{From SEO to GEO: Optimization Targets Under Generative Interfaces}
\label{sec:rw-seo-to-geo}

Recent work frames GEO as a distinct optimization problem from classic SEO, because the unit of exposure shifts from a ranked list of links to a \textit{synthesized answer} that selects, compresses, and attributes information to a small set of sources. In this framing, optimization targets include not only ranking and crawlability, but also being \textit{selected} and \textit{attributed} within the generative response, with potential engine-specific differences across systems and deployment contexts.

\subsection{What Generative Engines Prefer to Cite (Selection Bias and ``Extractability'')}
\label{sec:rw-selection-preferences}

Several GEO-oriented studies treat citation as a measurable behavior and argue that generative systems exhibit systematic preferences for certain source characteristics, such as information density, structured presentation, and third-party editorial authority. This supports analyzing selection as a distinct stage between retrieval and final generation, and motivates feature-based audits of the retrieved ``menu'' versus the cited ``order,'' as done in our Content DNA enrichment pipeline.

\subsection{Market Evidence and the Continued Role of Search Visibility}
\label{sec:rw-market-evidence}

Industry analyses report that generative search features can affect click-through behavior and redistribute visibility across domains, often in ways that reinforce the importance of being present in conventional search indices. While such reports are not controlled academic studies, they provide external motivation for measuring overlap between generative citations and SERP rankings and for treating rank depth (e.g., Top~10 vs. deep pagination) as a key determinant of exposure.
