...
"""
    ...
    # Target the header we found
    target_header = '## 1.7 Anatomy of a ChatGPT Response (Network-Instrumented)'
    ...
    if insert_pos != -1:
        ...
        # Note: We need to be careful with double-renumbering if headers repeat
        remainder = remainder.replace('## 1.7 Anatomy of a ChatGPT', '## 1.8 Anatomy of a ChatGPT')
        remainder = remainder.replace('## 1.8 Anatomy of a Gemini', '## 1.9 Anatomy of a Gemini')
        ...
        # Also fix the Localization header which might still be 1.8 or 1.9
        remainder = remainder.replace('## 1.8 Localization', '## 1.12 Localization') # Temporary unique number
        remainder = remainder.replace('## 1.12 Localization', '## 1.13 Localization') # Just to be safe
