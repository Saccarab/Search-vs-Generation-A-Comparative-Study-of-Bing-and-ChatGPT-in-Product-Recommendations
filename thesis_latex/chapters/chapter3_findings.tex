...
Japanese (Business), and Spanish (Plus); P048 (``Can I add a live translation app to calls?'') triggered on 3 of 6, 
...
This bucket of sources, which has a dominating concentration of product pages and listicles, is then utilized
within the response text interchangeably.
...
For each run, we pool the top-10 results across all fan-out queries and classify
 each unique URL as Cited-Inline or Not-Cited. A URL cited in run~1 but not run~2 contributes to both buckets.

Google Top-10 returns a consistent ten organic results per query but Bing Page~1 yields
a mean of only four organic results due to UI scraping limitations we have covered
 (Section~\ref{sec:bing-pagination-instability}), making the Bing pools considerably smaller. 
 Though this concentrates the comparison in the top ranks where citation probability is highest,
 so we end up comparing results of a similar ranking range.
\begin{table}[H]
\centering
...
Structured content features (tables, numbered lists, bullet points) are over-selected across almost all conditions. Gemini shows no significant freshness drift; this is consistent with its fan-out queries injecting year tokens that pre-filter the SERP for recency before selection occurs (Section~\ref{sec:freshness-subsection}). GPT Plus shows a consistent freshness preference that is significant against both Bing and Google independently, while GPT Business shows none. Vendor-owned pages are also consistently over-selected, suggesting that brands produce more structurally thorough content that LLMs prefer, possibly because commercial results lacking supporting detail are less likely to survive selection.
...
Product pages show flatter and less consistent drift profiles, making it hard to draw conclusive statements apart from numbered lists, which are over-selected in three of four conditions. Product pages are structurally more homogeneous, leaving less for either model to differentiate on.

\subsection{Cited vs Additional: Structural Comparison}
\label{sec:cited-vs-additional}

Both groups here were surfaced by ChatGPT: cited URLs appeared inline in the response text, additional URLs were shown as supplementary sources. Gemini does not have an ``additional'' category, so this comparison is GPT-only.
...
which \emph{products within that listicle} does the model extract, and how faithfully
...
Prior work suggests that content length and position within a page affect how much of a source the model actually processes. \textcite{petrovic2025grounding} report a per-query grounding budget of approximately 2{,}000 words for Gemini, with coverage dropping from 61\% for pages under 1K words to 13\% for pages over 3K; \textcite{petrovic2025filter} estimate that roughly one-third of source content survives into AI-generated citations. At the sub-page level, \textcite{indig2026attention} found that 44.2\% of ChatGPT citations originate from the first 30\% of page content.

Based on these findings, for a median listicle of over 2{,}000 words, products listed later may receive less model attention. Since attributing a product recommendation to a specific source is difficult when multiple links support the same claim, we filtered to claims backed by a single listicle (excluding 10.7\% of GPT and 34.5\% of Gemini claims).
...
The 2--3$\times$ bias multiplier is misleading. Vendors overwhelmingly list their own product at rank~\#1 (over 93\% of cases for both GPT and Gemini), and rank~\#1 is over-selected regardless of host status (Section~\ref{sec:reranking}). Controlling for position:
...
The residual advantage shrinks to +2.5--4.7\,pp, likely explained by the more favorable language host domains use when describing their own products. The 64--72\% host omission rate tells the same story from the other side: non-host rank~\#1 items are skipped at even higher rates (79.5--85.7\%).

\subsection{Semantic Fidelity}
\label{sec:semantic-fidelity}

The final question is whether models accurately reproduce the details of products they extract. Using the same LLM-as-judge pipeline, we score fidelity on a 1--5 scale by comparing each product claim against the source listicle text.
