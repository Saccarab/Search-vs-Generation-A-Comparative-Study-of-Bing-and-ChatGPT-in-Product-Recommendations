...
For each solo-cited claim, we ran an LLM-as-judge prompt through both GPT-5-mini and Gemini 2.5 Flash that performs fidelity scoring, listicle presence verification, rank extraction, and host-product detection in a single pass. Cross-model validation on all 1{,}116 claims confirmed structural field agreement at 93--100\% and fidelity scores within $\pm$1 in 93.9\% of matched pairs, consistent with the Content DNA labeler benchmark (Section~\ref{sec:labeler-fidelity}).
...
Reading comprehension is high: when the product is present in the source, both models score 4.71--4.81 out of 5.0. Fidelity scores were validated via cross-model agreement (GPT-5-mini vs Gemini 2.5 Flash on all 1{,}116 claims), with scores matching within $\pm$1 in 93.9\% of cases.
