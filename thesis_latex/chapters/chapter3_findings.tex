...
We have seen how fan-out queries are generated and how localization shapes their language. 
The next question is: where do the resulting citations come from?
We trace citations back to search engine results and examine how deep into the SERP models reach, and quantify how much falls outside conventional search entirely.

\subsection{Index Affinity: Which Search Engine?}
\label{sec:global-overlap}

For each LLM citation, we checked whether the exact URL appeared in our Bing or Google SERP scrape for the same fan-out queries that produced it. Tracking parameters (\texttt{utm\_*}, \texttt{fbclid}, etc.) are stripped before matching (Section~\ref{sec:bing-scraping}). The most consequential finding is that the search index behind ChatGPT's citations depends on the subscription tier.
...
Within these overlap ranges, \emph{which} index matters depends on the tier. \textbf{GPT Business} retrieves exclusively from Bing \parencite{openai2024enterprise}: 81.3\% Bing overlap, only 27.8\% Google. That 27.8\% is our baseline---the natural overlap that occurs because Bing and Google index many of the same pages. \textbf{GPT Personal} jumps to \textbf{64.6\%} Google overlap, a 36.8\,pp increase over that baseline, while its Bing overlap drops to 67.6\%. This is consistent with Personal drawing on Google in addition to Bing, as OpenAI's documentation implies by referencing ``third-party search providers'' in the plural \parencite{openai2024search}. Gemini retrieves from Google at \textbf{77.7\%} overlap---lower than one might expect for Google's own model, with the remaining 22.3\% invisible from the SERP.

For publishers, this means that optimizing only for Google is not 
...
data shows that Bing rankings matter for ChatGPT---especially for Business-tier users, where 81.3\% of citations come from Bing and only 27.8\% from Google. A brand that ranks well on Google but has no Bing presence may be effectively invisible to the largest ChatGPT deployment tier.

\subsection{Page Rank distribution of the Cited Links}
\label{sec:page-dist}

Our analysis of 237 runs per engine tier (79 queries $\times$ 3 runs) 
reveals that LLM citations are not concentrated on the first page of search results but are distributed deep into the SERP.
...
Business shows nearly 3$\times$ the Page~1 matches (1{,}468 vs 521), consistent with its higher Bing affinity. But the key observation is the long tail: hundreds of matches persist on Pages~4--10, and the decay curve has not reached zero by Page~16. Many GPT citations fall well beyond what human searchers would ever see. The Page~2 dip is a scraping artifact (Bing's UI pagination instability; see Section~2.2.1), not a model preference.
...
\begin{table}[H]
\centering
\caption{Page-1 rank distribution: LLM citation share vs.\ human CTR. LLM columns show matches at rank~$N$ as a share of Page-1 matches only (not all citations); Human CTR from \textcite{firstpagesage2026ctr} (Google organic, 2026) uses all clicks as the denominator. The columns are therefore not directly comparable in magnitude---see text for discussion.}
...
\begin{itemize}
    ...
    \item \textbf{LLMs ``look further down'' than humans}: Within Page~1, the shape is similar---monotonic decay from rank~1---but flatter for LLMs (top-3 = 45--50\% vs.\ 68.7\% for humans). An important caveat: the denominators differ. Human CTR sums to $\sim$94\% across ranks 1--10 because humans rarely go past Page~1---Page~1 \emph{is} effectively the entire search experience. The LLM columns sum to 100\% only because they are restricted to Page-1 matches by definition. As Section~\ref{sec:page-dist} showed, a large share of LLM citations come from Pages~2--16, so the true fraction of \emph{all} LLM citations landing on Page~1 is far lower than 100\%. The table compares the shape of rank preference within Page~1, not the overall reliance on it.
\end{itemize}
