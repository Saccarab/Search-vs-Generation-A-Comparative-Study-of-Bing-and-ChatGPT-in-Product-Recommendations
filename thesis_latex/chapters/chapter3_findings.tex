...
The overlap tables above show that \textbf{16--22\%} of LLM citations appear in neither
the Bing nor the Google SERPs we collected. Before examining what these links are,
it is worth considering why a citation might not match our scrapes.
...
will appear unmatched even though they exist in the search index.
...
The following tables combine all citation types
(cited + additional + rejected) that fall outside our SERP scrapes.
...
\noindent Since Personal uses both Bing and Google, the table below
shows how many Bing-invisible citations per domain were recovered
by our Google scrape, and how many remain invisible in both indices.
...
\noindent Both tables share Wikipedia, arxiv, and major tech
publications, but the differences between tiers are striking.
Business is almost strictly high-authority tech review sites
(tomsguide, theverge, wired, techradar), while Personal surfaces
consumer-oriented content that Business does not: \texttt{reddit.com}
(216 Bing-invisible, 126 recovered by Google),
\texttt{apps.apple.com} (139 Bing-invisible),
\texttt{chromewebstore.google.com} (54 Bing-invisible), and
\texttt{medium.com} (29)---none of which appear in the Business
invisible list at all. Personal's invisible profile includes app
store listings, user-generated content, and community posts,
while Business sticks to editorial and reference sources.

This contrast mirrors the tier differences we observed in
Section~\ref{sec:global-overlap}: even outside of search results,
the two ChatGPT tiers appear to draw from different retrieval
resources tailored to their subscription audience.

However, these counts include \textbf{rejected links}---URLs present
in the retrieval pool but never surfaced to the user---which are
overwhelmingly invisible (\textgreater90\% not in our SERPs).
To separate what the model actually showed users from what it
silently discarded, we now examine rejected links in detail.

%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{Rejected Links}
\label{sec:rejected}

Rejected links (Section~2.3.2)
are URLs present in the retrieval pool but never promoted to cited
or additional status. They are overwhelmingly invisible: only
\textasciitilde6--10\% appear in our Bing scrape and
\textasciitilde3--4\% in Google, compared to 57--86\% for cited
...
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsection{User-Facing Invisible Domains}
\label{sec:cited-invisible-final}

The tables in Section~\ref{sec:top-invisible-domains} included
rejected links, which inflated the counts for news and reference
domains. The table below shows only cited and additional links---the
invisible domains that actually appeared in the user-facing response.

\begin{table}[H]
\centering
\caption{User-facing invisible domains (cited + additional, top 10 per tier)}
\label{tab:cited-invisible-domains}
\small
\begin{tabular}{rlr@{\hskip 2em}rlr}
\toprule
\multicolumn{3}{c}{Business (280)} &
\multicolumn{3}{c}{Personal (653)} \\
\cmidrule(lr){1-3} \cmidrule(lr){4-6}
 & Domain & Count & & Domain & Count \\
\midrule
1 & en.wikipedia.org & 81 & 1 & reddit.com & 116 \\
2 & sfgate.com & 22 & 2 & apps.apple.com & 90 \\
3 & theverge.com & 22 & 3 & en.wikipedia.org & 65 \\
4 & tomsguide.com & 22 & 4 & chromewebstore.google.com & 29 \\
5 & androidcentral.com & 8 & 5 & tomsguide.com & 20 \\
6 & wired.com & 7 & 6 & facebook.com & 14 \\
7 & techradar.com & 5 & 7 & elevenlabs.io & 11 \\
8 & chromewebstore.google.com & 4 & 8 & blog.google & 10 \\
9 & geeky-gadgets.com & 4 & 9 & theverge.com & 10 \\
10 & timesofindia.indiatimes.com & 4 & 10 & support.google.com & 8 \\
\bottomrule
\end{tabular}
\end{table}

\noindent Once rejected links and product page / listicle noise are
stripped, the tier contrast sharpens further. Business is almost
entirely Wikipedia and tech editorial sites. Personal is led by
\texttt{reddit.com} (116), app stores (apps.apple.com, chromewebstore,
play.google.com), and even \texttt{facebook.com}---consumer and
community sources that do not appear in the Business list at all.

These invisible domains also split sharply by citation type.
Wikipedia is almost exclusively \emph{cited}: 75 of its 75 Business
invisible instances and 58 of 61 Personal instances appear inline,
never as supplementary material.
The editorial tier (sfgate, theverge) follows the same pattern.
By contrast, \texttt{reddit.com}'s 42 invisible instances are
overwhelmingly additional (40 additional vs.\ 2 cited), and
\texttt{apps.apple.com} skews similarly (26 additional vs.\ 7 cited).
In short, reference and editorial sources are invisible but still get
cited inline; UGC and app-store links are invisible \emph{and} stuck in
supplementary status.

\subsection{Cited vs.\ Additional: Systematic Bias}
\label{sec:cited-additional-bias}

Not every URL has an equal chance of being cited inline.
Of all unique additional URLs, \textbf{73--74\%} never appear as
cited in any run---they are permanently stuck in supplementary status.
The split is not random; certain page types and domains are
systematically excluded from inline citation.

\begin{table}[H]
\centering
\caption{Page types that skew toward always-additional
(unique URLs, Personal)}
\label{tab:always-additional}
\small
\begin{tabular}{lrrr}
\toprule
Type & Always-Additional & Cited & Ratio \\
\midrule
forum / UGC & 137 & 16 & 8.6$\times$ \\
documentation & 59 & 18 & 3.3$\times$ \\
marketplace / directory & 40 & 9 & 4.4$\times$ \\
listicle & 366 & 165 & 2.2$\times$ \\
\midrule
product\_page & 252 & 278 & 0.9$\times$ \\
reference (Wikipedia) & 8 & 28 & 0.3$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent Forum and UGC content (primarily \texttt{reddit.com}:
96~always-additional vs.\ 11~cited in Personal) almost never makes
it to inline citation. \texttt{github.com} (9 vs.\ 0 in Business)
and \texttt{youtube.com} (7 vs.\ 0) are similarly locked out.
On the other end, product pages and Wikipedia articles are more
likely to be cited inline than to remain supplementary.

This means the cited/additional boundary is partly systematic
(some types are effectively blocked from inline citation) and
partly stochastic---which is what the flip-flop analysis below
measures.

\subsection{Citation Flip-Flop Rate}
\label{sec:flip-flop}

For the URLs that \emph{can} cross the boundary, the model's
decision is stochastic: the same URL may be cited inline in one
run and demoted to additional in the next.
We measure this \textbf{flip-flop rate} at two scopes.
