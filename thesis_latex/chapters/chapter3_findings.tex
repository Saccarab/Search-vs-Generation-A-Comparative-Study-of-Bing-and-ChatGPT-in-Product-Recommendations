...
The overlap tables above show that \textbf{16--22\%} of LLM citations appear in neither the Bing nor the Google SERPs we collected. These ``invisible'' links represent sources that enter the model's grounding pool through channels opaque to conventional search.

\label{sec:invisible-conservative}
These rates are \textbf{upper bounds}. Our Bing scrape stops at Rank~200, but the page-level histogram (Section~\ref{sec:page-dist}) shows matches still accumulating at Pages~15--16 with no convergence, so some ``invisible'' URLs likely exist deeper in Bing's index. To filter out these cases, we exclude URLs classified as \texttt{product\_page} (31 for Business, 113 for Personal) from the invisible domain tables below---given the profile of our SERPs, results beyond our scrape depth should mostly be product pages or listicles. The exclusion is small relative to the total citation pool and serves as a final refinement rather than a major correction.
