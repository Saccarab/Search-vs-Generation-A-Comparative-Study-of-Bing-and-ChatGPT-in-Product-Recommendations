...
Table~\ref{tab:classifier-examples} shows the three probabilities parameters which we observed to be stable for a given prompt across different runs.
...
the same fan-out queries that produced it (Table~\ref{tab:global-overlap}).
...
The Page~2 dip on both Bing charts (Figure~\ref{fig:overlap-dist}) is consistent with the pagination
...
brand look-ups that naturally match fewer SERP results. The first fan-out query alone accounts for more citations than any single later query.
...
further establish that a solid SEO carries a lot of weight for AI visibility.
...
To see how much, we compared the structural profile of overlapped
and invisible links across both cited and additional categories. GPT Business has our highest index
coverage (83.7\%), so we use it as the reference case (Table~\ref{tab:invisible-type-breakdown}).

\begin{table}[H]
\centering
\caption{GPT Business: structural profile of overlapped vs.\ invisible links}
\label{tab:invisible-type-breakdown}
\small
\begin{tabular}{lrrrr}
\toprule
 & \multicolumn{2}{c}{Overlapped (3{,}768)} & \multicolumn{2}{c}{Invisible (689)} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Type & Count & \% & Count & \% \\
\midrule
listicle & 1{,}757 & 46.6\% & 182 & 26.4\% \\
product\_page & 1{,}485 & 39.4\% & 187 & 27.1\% \\
reference (Wikipedia, etc.) & 3 & 0.1\% & 91 & 13.2\% \\
news / editorial & 74 & 2.0\% & 114 & 16.5\% \\
app store listing & 49 & 1.3\% & 7 & 1.0\% \\
other & 400 & 10.6\% & 108 & 15.7\% \\
\bottomrule
\end{tabular}
\end{table}

\noindent Overlapped links are 86.0\% product pages and
listicles, the page types that dominate product-recommendation SERPs.
In the invisible set, those same types still account for 53.6\%, while
reference and news/editorial pages jump from 2.0\% to 29.8\%.
...
Tables~\ref{tab:ent-invisible-combined} and~\ref{tab:pers-invisible-combined} break down invisible domains by source category for each tier.
...
different runs (Table~\ref{tab:flipflop-cross}).
...
%-----------------------------------------------------------------------
\section{Structural Profiles \& Selection Drift}
...
We profiled approximately 6{,}500 URLs with our enrichment pipeline (Section~\ref{sec:labeling-pipeline}):
...
smaller and concentrated in the top ranks (Table~\ref{tab:drift-listicles}).
\begin{table}[H]
\centering
...
Based on these findings, for a median listicle of over 2{,}000 words (Table~\ref{tab:content-size}), products listed later may receive less model attention. 
...
For each solo-cited claim, we ran an LLM-as-judge prompt (Appendix~\ref{app:fidelity-prompt}) through both GPT-5-mini and Gemini 2.5 Flash that performs fidelity scoring, listicle presence verification, rank extraction, and host-product detection in a single pass. Cross-model validation on all 1{,}110 claims confirmed structural field agreement at 93--100\% and fidelity scores within $\pm$1 in 93.9\% of matched pairs, consistent with the structural profile labeler benchmark (Section~\ref{sec:labeler-fidelity}).
...
Table~\ref{tab:listicle-rank-bias} compares each product's position in the model's response against its rank in the source listicle. Exact rank alignment is low: Gemini matches in 29.5\% of cases and GPT in 27.9\%, so models \textbf{re-rank} rather than preserve source order. Despite re-ranking, they strongly over-select from the top: the \#1 item is selected at 1.7--2.5$\times$ the uniform baseline, and the top-5 account for 76--81\% of all selections. This mirrors the SERP-level position bias (Section~\ref{sec:position-bias}): positional over-selection appears at two nested levels, SERP rank and within-listicle rank. Items further down a long listicle may simply never enter the context window the model processes.

\subsection{Self-Promotion Bias \& Host Exclusion}
\label{sec:self-promo}

Roughly 62\% of listicles in our dataset are vendor-owned, so we wanted to see if models apply any bias for or against the products of the host domain (Table~\ref{tab:host-exclusion}).
