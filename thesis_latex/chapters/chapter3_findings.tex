...
Since both systems issue multiple fan-out queries per run, we checked whether citations are drawn evenly across them or concentrated in one. For ChatGPT, which issues two fan-out queries per run, overlap is nearly identical between the two queries across all tiers (Table~\ref{tab:gpt-per-query}). Neither query consistently dominates; citations are drawn from both equally.
